...
public class AccReadRepository {
    ...
    public AccRecord getAccByManifestId(BigInteger accManifestId) {
        return dslContext.select(ACC.fields()).from(ACC).join(ACC_MANIFEST).on(ACC.ACC_ID.eq(ACC_MANIFEST.ACC_ID)).where(ACC_MANIFEST.ACC_MANIFEST_ID.eq(ULong.valueOf(accManifestId))).fetchOptionalInto(AccRecord.class).orElse(null);
    }

    public AccManifestRecord getAllExtensionAccManifest(BigInteger releaseId) {
        return dslContext.select(ACC_MANIFEST.fields()).from(ACC_MANIFEST).join(ACC).on(ACC_MANIFEST.ACC_ID.eq(ACC.ACC_ID)).where(and(ACC_MANIFEST.RELEASE_ID.eq(ULong.valueOf(releaseId)), ACC.TYPE.eq(CcACCType.AllExtension.name()))).fetchOptionalInto(AccManifestRecord.class).orElse(null);
    }

    public AccManifestRecord getAccManifest(BigInteger accManifestId) {
        return dslContext.selectFrom(ACC_MANIFEST).where(ACC_MANIFEST.ACC_MANIFEST_ID.eq(ULong.valueOf(accManifestId))).fetchOptionalInto(AccManifestRecord.class).orElse(null);
    }

    public List<CcList> getBaseAccList(BigInteger accManifestId, BigInteger releaseId) {

        ULong defaultModuleSetReleaseId = null;
        ModuleSetReleaseRecord defaultModuleSetRelease = dslContext.selectFrom(MODULE_SET_RELEASE).where(and(MODULE_SET_RELEASE.IS_DEFAULT.eq((byte) 1), MODULE_SET_RELEASE.RELEASE_ID.eq(ULong.valueOf(releaseId)))).fetchOne();
        ...
        return dslContext.select(ACC_MANIFEST.ACC_MANIFEST_ID, ACC.ACC_ID, ACC.GUID, ACC_MANIFEST.DEN, ACC.DEFINITION, ACC.DEFINITION_SOURCE, ACC.OBJECT_CLASS_TERM, ACC.OAGIS_COMPONENT_TYPE, ACC.STATE, ACC.IS_DEPRECATED, ACC.LAST_UPDATE_TIMESTAMP, MODULE.PATH, MODULE.NAME, appUserOwner.LOGIN_ID, appUserUpdater.LOGIN_ID, LOG.REVISION_NUM, LOG.REVISION_TRACKING_NUM, RELEASE.RELEASE_NUM).from(ACC_MANIFEST).join(ACC).on(ACC_MANIFEST.ACC_ID.eq(ACC.ACC_ID)).join(LOG).on(ACC_MANIFEST.LOG_ID.eq(LOG.LOG_ID)).join(RELEASE).on(ACC_MANIFEST.RELEASE_ID.eq(RELEASE.RELEASE_ID)).join(appUserOwner).on(ACC.OWNER_USER_ID.eq(appUserOwner.APP_USER_ID)).join(appUserUpdater).on(ACC.LAST_UPDATED_BY.eq(appUserUpdater.APP_USER_ID)).leftJoin(MODULE_ACC_MANIFEST).on(and(ACC_MANIFEST.ACC_MANIFEST_ID.eq(MODULE_ACC_MANIFEST.ACC_MANIFEST_ID), MODULE_ACC_MANIFEST.MODULE_SET_RELEASE_ID.eq(defaultModuleSetReleaseId))).leftJoin(MODULE).on(MODULE_ACC_MANIFEST.MODULE_ID.eq(MODULE.MODULE_ID)).where(ACC_MANIFEST.ACC_MANIFEST_ID.in(accManifestIdList)).fetch().map(e -> {
            ...
            ccList.setLastUpdateTimestamp(Date.from(e.getValue("last_update_timestamp", LocalDateTime.class).atZone(ZoneId.systemDefault()).toInstant()));
