...
-  Free form text filtering based on CCs’ *DEN* (dictionary entry name),
   *Definition*, *Module* or a combination of them. The matching is case
   ...
   -  The *Module* field allows for filtering based on the physical file
      ...
      “Model\\\\Platform\\\\2_4\\\\Common” folder, nouns are serialized
      to the respective noun module (e.g.
      “Model\\\\Nouns\\\\PurchaseOrder”). To search only shared
      components, the user may enter “common” in the *Module* field. Or,
      to search only about nouns, the user may enter “nouns” in the
      *Module* field.
...
Note: Search returns the number of matching nodes in 10 depth from the
root node. The number of matching nodes is depicted next to the search
input field followed by a plus icon. This icon indicates that there
might be more nodes in the tree. Should you want to search deeper in the
tree, you should expand a specific node and use the searching function
again. In this case, search results will include the number of matching
nodes in 10 depth from the expanded node.
